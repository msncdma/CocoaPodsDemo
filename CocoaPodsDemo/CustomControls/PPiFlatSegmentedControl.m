...
-(void)updateSegmentsFormat{
    ...
    for (UIButton *segment in self.segments){
        ...
        if([self.segments indexOfObject:segment]==self.currentSelected){
            //Selected-one
            if(self.selectedColor)[segment setBackgroundColor:self.selectedColor[segment.tag-250] forUIControlState:UIControlStateNormal];
            ...
            //Non selected
            if(self.color)[segment setBackgroundColor:self.color[segment.tag - 250] forUIControlState:UIControlStateNormal];
...
-(void)setSelectedColor:(NSMutableArray *)selectedColor{
    _selectedColor=selectedColor;
    [self updateSegmentsFormat];
}
-(void)setColor:(NSMutableArray *)color{
    _color=color;
    [self updateSegmentsFormat];
}
